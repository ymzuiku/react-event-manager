...
class Event extends React.PureComponent {
  ...
  componentDidMount() {
    ...
    if (typeof onDidMount === 'function') {
      // 用于在渲染后,就获得refs对象结构
      onDidMount({ ref: this.ref, value: this.state.value, name, update: this.update });
    }
  }
  ...
  handleOnEvent = (e, eventArgs, eventName) => {
    ...
    if (typeof onEvent === 'function') {
      // 统一回调到Form的onChange中
      onEvent({ ref: this.ref, value, name, eventArgs, update: this.update, eventName });
...
export default class extends React.PureComponent {
  ...
  handleOnEvent = ({ ref, value, name, update, eventName, eventArgs, isFromDidMount }) => {
